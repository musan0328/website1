<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/destyle.min.css">
    <link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/top.css">
    <title>Document</title>
</head>
<body>
    <header>
       <nav id="gnav">
           <div>
               <a href="#"><img src="<?php echo get_template_directory_uri(); ?>/images/onigiri_tenmusu.png" alt="ロゴ" class="logo-img"></a>
           </div>
           <div class="inner-area">
                    <div class="header-tell">
                        <p class="tell">
                            <img src="<?php echo get_template_directory_uri(); ?>/images/tell-icon.png" alt="電話アイコン" class="tell-icon">
                            <span>0120-123-456</span>
                        </p>
                        <ul class="inner">
                                <li class="inner_child"><a href="#" class="inner_text">トップページ</a></li>
                                <li class="inner_child"><a href="#" class="inner_text">1ページ目</a></li>
                                <li class="inner_child"><a href="#" class="inner_text">2ページ目</a></li>
                                <li class="inner_child"><a href="#" class="inner_text">3ページ目</a></li>
                                <li class="inner_child"><a href="#" class="inner_text">ニュースページ</a></li>
                        </ul>
                    </div>
                    <div class="header-box">
                            <div class="sns-area">
                                <ul class="sns-list">
                                    <li><a href=""><img src="<?php echo get_template_directory_uri(); ?>/images/insta.png" alt="インスタアイコン" class="insta-icon"></a></li>
                                    <li><a href=""><img src="<?php echo get_template_directory_uri(); ?>/images/twitter.png" alt="twitterアイコン" class="twitter-icon"></a></li>
                                </ul>
                            </div>
                    </div>
                </div>
                <div class="contact-area">
                    <a href="#" class="contact-link">
                        <div class="contact-title">
                            <span class="contact-icon"></span>
                                <p class="contact-text">
                                    <span class="contact-text1">お問い合わせ</span>
                                    <span class="contact-text1">面談予約</span>
                                </p>
                        </div>
                    </a>
            </div>
       </nav>
    </header>